...
Функция ВыполнитьКоманду(Знач Параметры) Экспорт
	...
	Если КодВозврата <> 0 И мИсключениеПриОшибкеВыполненияКоманды Тогда
		Лог.Ошибка("Получен ненулевой код возврата " + КодВозврата);
		ВызватьИсключение ВыводКоманды();
	Иначе
...
Процедура ВыполнитьПроверкуПрименимостиРасширения(ИмяРасширения = "") Экспорт
	...
	Параметры.Добавить("/CheckCanApplyConfigurationExtensions");
...
//  НомерВерсии  		  - Число -  Номер версии в истории в хранилище конфигурации, если 0 или меньше, то берется последняя версия.
//
Процедура ЗагрузитьКонфигурациюИзХранилища(Знач СтрокаСоединения, Знач ПользовательХранилища, Знач ПарольХранилища = "", Знач НомерВерсииХранилища = 0, ИмяРасширения = Неопределено) Экспорт

	ПараметрыСвязиСБазой = СформироватьПараметрыСвязиСБазойДляПолучениеВерсииКонфигурацииИзХранилища(СтрокаСоединения, ПользовательХранилища, ПарольХранилища, НомерВерсииХранилища, ИмяРасширения);
...
Функция СформироватьПараметрыСвязиСБазойДляПолучениеВерсииКонфигурацииИзХранилища(СтрокаСоединения, ПользовательХранилища, ПарольХранилища, НомерВерсииХранилища, ИмяРасширения = "") 
