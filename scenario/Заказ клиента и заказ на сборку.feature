...
* Выбор номенклатуры с остатком и без характеристик
	И я выполняю код встроенного языка на сервере
		...
		ВыборкаЗапроса = Запрос.Выполнить().Выбрать();
					...
					КонецЕсли;
		"""
	И я запоминаю значение выражения "Объект.ЗначениеНаСервере" в переменную "НоменклатураТовара"
...
* Заполнение шапки документа
	...
	И в таблице 'Список' я перехожу к строке:
		| "Наименование"             |
		| "Громов Борис Юрьевич уд!" |
	И я нажимаю на кнопку с именем 'ФормаВыбрать'
	Тогда элемент формы с именем 'Партнер' стал равен "Громов Борис Юрьевич"

	* Переменная Клиент
		И я запоминаю значение поля с именем "Партнер" как "Клиент"
...
* Переменные Номер и Сумма
	И я запоминаю значение поля с именем "Номер" как "НомерЗаказа" глобально
	И я запоминаю представление элемента формы с именем 'СуммаВсегоСНДС' как "Сумма"

* Ссылка на документ
	И я нажимаю сочетание клавиш "Ctrl+F11"
	Если появилось предупреждение, тогда
		И я нажимаю на кнопку с именем 'Button0'
	Когда открылось окно "Получение ссылки"
	И я нажимаю на кнопку с именем 'btnGetURL'
	И я запоминаю значение поля с именем "CurrentNavPointContent" как "НавигационнаяСсылка" глобально
...
* Проверка создания документа
	...
	* Проверка наличия документа в списке заказов
		...
		И таблица 'Список' содержит строки:
			| 'Номер'                                    | 'Сумма'   | 'Клиент'   |
			| '<b><colorstyle -46>$$НомерЗаказа$$</></>' | '$Сумма$' | '$Клиент$' |


Сценарий: Проверка печатной формы заказа на отгрузку
...
* Формирование печатной формы Заказа на отгрузку
	И я нажимаю на кнопку с именем 'ПодменюПечатьОбычное_2'
	Тогда открылось окно "Заказ клиента $$НомерЗаказа$$ от *"


//ЗАМЕТКИ НА БУДУЩЕЕ
//1) СДЕЛАТЬ КРАСОТУ: ПЕРЕНЕСТИ ВЫБОРКУ В НАЧАЛО, ПОДПРАВИТЬ ВСЯКИЕ НЕДОЧЁТЫ (БУДУЩИЙ Я, УДАЧИ)
//2) СДЕЛАТЬ ПОЛУЧЕНИЕ ССЫЛКИ ЧЕРЕЗ КОД