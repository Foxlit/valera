...
Процедура ЗаполнитьЗначенияПеременных()
	...
	ПараметрыРаботы.Вставить("ПутьКРезультатуДымовыхтестов",
		"E:\Update1cUT\Vanessa\repo\features\BuildStatus.log");
	...
	ПараметрыРаботы.Вставить("ПутьКРепозиторию", "E:\Update1cUT\Vanessa\repo");

	ПараметрыРаботы.Вставить("RASSERVER", "localhost");
	ПараметрыРаботы.Вставить("Версия1С", "8.3.22.1704");
	ПараметрыРаботы.Вставить("RASPORT", "1545");
	ПараметрыРаботы.Вставить("RACPATH", "C:\Program Files\1cv8\Версия1С\bin\rac.exe");
	ПараметрыРаботы.RACPATH = СтрЗаменить(ПараметрыРаботы.RACPATH, "Версия1С", ПараметрыРаботы.Версия1С);
	ПараметрыРаботы.Вставить("СообщениеБлокировки", "Уважаемые пользователи, в данный 
	|момент проводится плановое обновление базы данных.");
	ПараметрыРаботы.Вставить("КодБлокировки", "allowplz");
	ПараметрыРаботы.Вставить("КоличествоСекундПередСтартомБлокировки", 10);
	ПараметрыРаботы.Вставить("СтрокаСоединенияБазыДанных", "/Ssrv-1c-dev/trade11_test");
	ПараметрыРаботы.Вставить("ИмяСлужбыСервера", "1C:Enterprise 8.3 Server Agent (x86-64)");	
...
Функция ОтключитьБазуОтХранилища(ИмяРасширения = "")
	...
	Конфигуратор.УстановитьКлючРазрешенияЗапуска(ПараметрыРаботы.КодБлокировки);	
...
Процедура ЗабратьДанныеИзГит(Отказ)
    Попытка
		ОтправитьСообщение("Получаем данные из Git");
		
		КаталогРепозитория = ПараметрыРаботы.ПутьКРепозиторию;
...
Процедура УдалитьВсеСоединенияСБазой(Отказ)
	Подключение = Новый ПодключениеMSSQL(ПараметрыРаботы.АдресСУБД,
			ПараметрыРаботы.ИмяПользователяСУБД,
			ПараметрыРаботы.ПарольСУБД);
	Попытка
		...
		РаботаССУБД.УстановитьОднопользовательскийРежимБазы(ПараметрыРаботы.ИмяБазы);
		
		СообщениеПереводаВОднопользовательскийРежим = СтрШаблон(
			"База %1 переведена в однопользовательский режим", 
			ПараметрыРаботы.ИмяБазы);
	...
	Исключение
		...
		СообщениеПереводаВОднопользовательскийРежим = СтрШаблон(			
			"Не удалось перевести базу %1 в однопользовательский режим: %2",
			 ПараметрыРаботы.ИмяБазы, 
			 ОшибкаВосстановления);
	...
	ЗаписатьЛог(СообщениеПереводаВОднопользовательскийРежим);
	ОтправитьСообщение(СообщениеПереводаВОднопользовательскийРежим);	
КонецПроцедуры
...
Функция СнятиеБлокировкиСеансов()
	Возврат "vrunner session unlock --ras ""RASSERVER"":""RASPORT"" --rac ""RACPATH"" --db ""RUNNER_IBNAME"" --db-user ""RUNNER_DBUSER"" --db-pwd ""RUNNER_DBPWD"" --uccode ""RUNNER_uccode"""; // BSLLS:LineLength-off
КонецФункции
...
Функция ЗаменитьПараметрыСтрокиВыполнения(Знач СтрокаВыполнения)
	...
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RASSERVER", ПараметрыРаботы.RASSERVER);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RACPATH", ПараметрыРаботы.RACPATH);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RASPORT", ПараметрыРаботы.RASPORT);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RUNNER_IBNAME", ПараметрыРаботы.ИмяБазы);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RUNNER_DBUSER", ПараметрыРаботы.ИмяПользователя1с);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RUNNER_DBPWD", ПараметрыРаботы.ПарольПользователя1с);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "LOCKSTARTAT", 
	ПараметрыРаботы.КоличествоСекундПередСтартомБлокировки);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "LOCKMESSAGE", ПараметрыРаботы.СообщениеБлокировки);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RUNNER_v8version", ПараметрыРаботы.Версия1С);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "RUNNER_uccode", ПараметрыРаботы.КодБлокировки);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "IBCONNECTION", ПараметрыРаботы.СтрокаСоединенияБазыДанных);
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "ИмяСлужбыСервера", ПараметрыРаботы.ИмяСлужбыСервера);

	Возврат СтрокаВыполнения;
	
КонецФункции

Процедура СнятьБлокировкуСеансов(Отказ)
	Попытка
	...
	Исключение
		Отказ = Истина;
		ТекстСообщения = "Не удалость снять блокировки сеансов:" + ОписаниеОшибки();
		ЗаписатьЛог(ТекстСообщения);
	КонецПопытки;	
КонецПроцедуры	
...
Процедура Регламент()
	Попытка
		...
		Отказ = Ложь;
		ОтправитьСообщение("Завершение активных");
		//Завршаем сеансы 1с через RAC сервер
		ИсполнитьКоманду(ЗавершениеАктивныхСеансов());
		// Ждём 5 секунд, иначе остановка службы может начаться раньше, чем RAC закончит работу
		Приостановить(5000);
		//Сеансы удаляются через SQL
		УдалитьВсеСоединенияСБазой(Отказ);
		ОчисткаСтарыхБэкапов();
		ЗабратьДанныеИзГит(Отказ);
		//Восстановление происходит посредством SQL
		ВосстановитьБазуЗапускаТестов(Отказ);
		//Снятие блокировки сеансов средствами RAC
		СнятьБлокировкуСеансов(Отказ);
		Если НЕ Отказ Тогда
