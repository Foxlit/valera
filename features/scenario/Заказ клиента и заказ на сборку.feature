...
* Штрихкод распоряжения
	И я выполняю код встроенного языка на сервере
		"""bsl
			Штрихкод = ШтрихкодированиеПечатныхФорм.ЧисловойКодПоСсылке("$ИмяПеременной$");
		"""
	И я запоминаю значение выражения "Штрихкод" в переменную "ШтрихкодРаспоряжения"

И Я запоминаю значение выражения 'вцйай' в переменную "МояПеременная"
И я выполняю код встроенного языка на сервере
"""
...
* Выбор заказа
	...
	И в поле с именем 'InputFld' я ввожу текст "132368885697765136910356161490400402944"
