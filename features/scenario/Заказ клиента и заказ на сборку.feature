...
Сценарий: сброс пароля пользователя
	...
	"""
	И я подключаю TestClient "Этот клиент" логин "ТенниковА" пароль "1"
...
* Выбор номенклатуры с остатком и без характеристик для поиска
	И я выполняю код встроенного языка на сервере
		"""bsl
...
* Отдельно слкад для заполнения поля в документе
	И я выполняю код встроенного языка на сервере
	"""bsl
		Объект.ЗначениеНаСервере = НЕОПРЕДЕЛЕНО;
		Объект.ЗначениеНаСервере = ПланыВидовХарактеристик.ПредопределенныеЗначения.СкладОптовый.Значение;
	"""
	И Я запоминаю значение выражения 'Объект.ЗначениеНаСервере' в переменную "СкладДляЗаказа"


* Выбор номенклатуры с остатком и без характеристик для склада
	И я выполняю код встроенного языка на сервере
		"""bsl
			Объект.ЗначениеНаСервере = НЕОПРЕДЕЛЕНО;
			Запрос = Новый Запрос(
			...
			ВыборкаЗапроса = Запрос.Выполнить().Выбрать();
				Если ВыборкаЗапроса.Следующий() Тогда
					Объект.ЗначениеНаСервере = СтрШаблон("%1: %2, ", ВыборкаЗапроса.Артикул, ВыборкаЗапроса.Номенклатура);
				КонецЕсли;
		"""
	И я запоминаю значение выражения "Объект.ЗначениеНаСервере" в переменную "НоменклатураТовараДляСклада" глобально
...
* Ячейка с остатком номенклатуры
	И я выполняю код встроенного языка на сервере
		"""bsl
			Объект.ЗначениеНаСервере = НЕОПРЕДЕЛЕНО;
			Запрос = Новый Запрос (
			...
			|	ТоварыВЯчейкахОстатки.ВНаличииОстаток КАК КоличествоТовара
			|ИЗ
			|	РегистрНакопления.ТоварыВЯчейках.Остатки КАК ТоварыВЯчейкахОстатки
			|ГДЕ
			|	НЕ ТоварыВЯчейкахОстатки.Номенклатура.ВидНоменклатуры.ИспользоватьСерии
			|	И ТоварыВЯчейкахОстатки.ВНаличииОстаток >= 5
			|	И ТоварыВЯчейкахОстатки.Характеристика = &Характеристика
			|	И ТоварыВЯчейкахОстатки.Ячейка.Владелец = &Склад
			|	И ТоварыВЯчейкахОстатки.Номенклатура 
			|	В
			|  (ВЫБРАТЬ ПЕРВЫЕ 1
			|		РаспределениеЗапасов.Номенклатура КАК Номенклатура
			|	ИЗ
			...
			|		И РаспределениеЗапасов.Склад = &Склад
			|		И РаспределениеЗапасов.Характеристика = &Характеристика
			|		И ТоварыВЯчейкахОстатки.Ячейка.ТипСкладскойЯчейки = &ТипСкладскойЯчейки)");
					...
					ВыборкаЗапроса = Запрос.Выполнить().Выбрать();
						Если ВыборкаЗапроса.Следующий() Тогда
							Объект.ЗначениеНаСервере = Строка(ВыборкаЗапроса.Ячейка);
						КонецЕсли;
		"""
	И Я запоминаю значение выражения 'Объект.ЗначениеНаСервере' в переменную "ЯчейкаТовара" глобально
...
* Заполнение данных для доставки
	...
	Тогда открылось окно "Партнеры (Поставщики и Перевозчики)"
	И в таблице 'Список' я перехожу к строке:
		| "Наименование"  |
		| "Деловые линии" |
	И в таблице 'Список' я нажимаю на кнопку с именем 'ФормаВыбрать'
	И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
	И я жду закрытия окна "Реализация товаров и услуг $НомерРеализации$ от *" в течение 20 секунд
	Если элемент формы "ФормаПровестиИЗакрыть2" отсутствует на форме тогда
...
* Доставка
	...
	И я запоминаю значение поля "Номер" как "НомерРеализации"
	И В текущем окне я нажимаю кнопку командного интерфейса "Доставка"
	Если открылось окно "1С:Предприятие" тогда
		И я нажимаю на кнопку с именем 'Button0'
	И из выпадающего списка с именем 'СпособДоставки' я выбираю точное значение "Перевозчик с нашего склада до клиента"
...
* Штрихкод документа
	...
	"""bsl
		Объект.ЗначениеНаСервере = НЕОПРЕДЕЛЕНО;
		НС = "$$НавигационнаяСсылкаОкна$$";

		Ссылка = ДоработкиДобавленный.ПолучитьСсылкуИзНавигационной(НС);
		Объект.ЗначениеНаСервере = ШтрихкодированиеПечатныхФорм.ЧисловойКодПоСсылке(Ссылка);
	"""

	И Я запоминаю значение выражения 'Объект.ЗначениеНаСервере' в переменную "ШтрихкодДокумента" глобально
...
* Отбор товаров
	...
	* Выбор заказа
		...
		И в поле с именем 'InputFld' я ввожу текст "$$ШтрихкодДокумента$$"
		...
		И я перехожу к закладке с именем 'ГруппаТовары'
		Когда открылось окно "Отбор товаров"
		И в таблице 'Товары' я нажимаю на кнопку с именем 'ТоварыУбратьФильтр'
		И в таблице 'Товары' я нажимаю на кнопку с именем 'ТоварыОтсканироватьОтправителя'
		Тогда открылось окно "Складские ячейки"
		И в таблице 'Список' в дополнение формы с именем 'СписокСтрокаПоиска' я ввожу текст "$$ЯчейкаТовара$$"
		И пауза 10
		И в таблице 'Список' я перехожу к строке:
			| 'Адрес'                                     |
			| '<b><colorstyle -46>$$ЯчейкаТовара$$</></>' |
		И я нажимаю на кнопку с именем "ФормаВыбрать"
		...
		Тогда таблица 'Товары' стала равной:
			...
			| '$$НоменклатураТовараДляСклада$$'  | '5 / 5 шт (по распоряжению 5), $$ЯчейкаТовара$$' |
		И я нажимаю на кнопку с именем 'ФормаЗавершитьОперацию'
		И я нажимаю на кнопку с именем 'Button0'
		Тогда открылось окно "Завершение сессии"
		И я нажимаю на кнопку с именем 'Button0'

* Отгрузка товаров
	Когда открылось окно "Выберите операцию"
	И я нажимаю на кнопку с именем 'ОтгрузкаТоваров'
	* Выбор заказа
		Когда открылось окно "Отгрузка товаров"
		И я нажимаю на кнопку с именем 'ФормаВвестиШтрихкод'
		Тогда открылось окно "Введите штрихкод"
		И в поле с именем 'InputFld' я ввожу текст "$$ШтрихкодДокумента$$"
		И я нажимаю на кнопку с именем 'OK'
		...
		Тогда таблица 'Товары' стала равной:
			| 'Верхняя строка'                   | 'Нижняя строка'             |
			| '$$НоменклатураТовараДляСклада$$'  | '5 / 5 шт (по накладной 5)' |
		И я нажимаю на кнопку с именем 'ФормаЗавершитьОперацию'
		И я нажимаю на кнопку с именем 'Button0'
		Тогда открылось окно "Завершение сессии"
