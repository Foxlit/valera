...
* Ссылка на документ
	...
	И я выполняю код встроенного языка на сервере
	"""bsl
		НС = ПолучитьНавигационнуюСсылку(ЭтотОбъект);
		ДоработкиДобавленный.ПолучитьСсылкуИзНавигационной(НС);
  		Штрихкод = ШтрихкодированиеПечатныхФорм.ЧисловойКодПоСсылке(Ссылка);
	"""
	И Я запоминаю значение выражения 'Штрихкод' в переменную "ШтрихкодДокумента" глобально
