...
* Удаление глобальных переменных
	И я удаляю переменную 'НавигационнаяСсылкаОкна'
...
* Заполнение шапки документа
	...
	* Переменная Клиент
<<<<<<< Updated upstream
		И я запоминаю значение поля с именем "Контрагент" как "Контрагент"
...
<<<<<<< Updated upstream
			| 'Номер'                                    | 'Сумма'   | 'Контрагент' |
			| '<b><colorstyle -46>$$НомерЗаказа$$</></>' | '$Сумма$' | '$Контрагент$'   |		
