...
Сценарий: выключение уведомлений
	...
	"""bsl
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|  Рег.ВидОповещения КАК ВидОповещения,
		|  Рег.Пользователь КАК Пользователь,
		|  Рег.СрокОповещения КАК СрокОповещения,
		|  Рег.Периодичность КАК Периодичность,
		|  Рег.ЭлектроннаяПочта КАК ЭлектроннаяПочта,
		|  Рег.СМС КАК СМС,
		|  ЛОЖЬ КАК Напоминание,
		|  Рег.Хранилище КАК Хранилище,
		|  Рег.Сумма КАК Сумма
		|ИЗ
		|  РегистрСведений.CRM_ОповещенияПользовательскиеНастройки КАК Рег
		|ГДЕ
		|  Рег.Напоминание";
		...
		МЗ = РегистрыСведений.CRM_ОповещенияПользовательскиеНастройки.СоздатьМенеджерЗаписи();
