...
* Второй товар
	...
	И пауза 6
	И я нажимаю на кнопку с именем 'СписокРасширенныйПоискНоменклатураВыбратьЗначениеНоменклатуры'
	И я жду закрытия окна "Номенклатура" в течение 10 секунд
	И в таблице 'Товары' я активизирую поле с именем 'ТоварыКоличествоУпаковок'
...
* Проверка формирования печатной формы УПД
	...
	И я жду когда в табличном документе "ТекущаяПечатнаяФорма" заполнится ячейка "R1C2:R3C4" в течение 10 секунд
	И в табличном документе "ТекущаяПечатнаяФорма" ячейка с адресом "R1C2:R3C4" равна "Универсальный передаточный документ"
	И в табличном документе "ТекущаяПечатнаяФорма" ячейка с адресом "R6C3" равна "1"
