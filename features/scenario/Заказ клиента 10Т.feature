...
Сценарий: Заказ клиента на 10 позиций
	...
	* Заполнение информации заказчика
		...
		И я снимаю флаг с именем 'OnlyCurrentGroup'
		...
		Когда открылось окно "Соглашения об условиях продаж"
		И в таблице 'Список' я перехожу к строке:
		...
		И в таблице 'Список' я перехожу к строке:
			| "Наименование"                      |
			| "Склады поставщиков (dropshipping)" |
...
* Счёт организации
	...
	И пауза 3
	И я сохраняю навигационную ссылку текущего окна в переменную "СсылкаЗаказа10"
	Когда поле с именем "Номер" заполнено тогда
		И я запоминаю значение поля с именем "Номер" как "НомерЗаказа10"
=======
	* Счёт организации
		И я перехожу к закладке с именем 'ГруппаОсновное'
		И я нажимаю на гиперссылку с именем 'ДекорацияЭтапыОплаты'
...
>>>>>>> Stashed changes
	...
	* Отгрузка товара
		...
		Если текущее окно имеет заголовок "Обеспечение заказа" Тогда
...
* Составление отчета реализации
	...
	* Доставка
		...
		И в таблице 'Список' я перехожу к строке:
			| "Дата регистрации" | "Комментарий"                                             | "Наименование"  | "Основной менеджер" |
			| "30.01.2020"       | "#Создан автоматически при импорте банковских документов" | "Деловые линии" | "Сигеев Владислав"  |
		И в таблице 'Список' я нажимаю на кнопку с именем 'ФормаВыбрать'
...
* Проверка формирования печатной формы УПД
	...
	И я закрываю все окна клиентского приложения кроме "Заказ клиента $НомерЗаказа10$ от *"

* Составление печатной формы заказа
	Если текущее окно "не равно" ожидаемому окну "Заказ клиента $НомерЗаказа10$ от *" Тогда
		И Я открываю навигационную ссылку "$СсылкаЗаказа10$"
...
* Отправка печатной формы заказа по почте
	...
	Тогда элемент формы с именем 'СписокПолучателейПисьма' стал равен "Громов Борис Юрьевич <gromov@fitathlon.ru>"
		
=======
	* Перменная СсылкаЗаказа10
		И я сохраняю навигационную ссылку текущего окна в переменную "СсылкаЗаказа10"
	...
	* Составление отчета реализации
		...
		И в таблице 'Список' я перехожу к строке:
			| 'Наименование'                                                    |
			| '<b><colorstyle -46>Деловые</></> <b><colorstyle -46>линии</></>' |
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		И я меняю значение переключателя с именем 'Плательщик' на "За счет клиента"
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
		И я жду закрытия окна "Реализация товаров и услуг FATR-004093 от *" в течение 20 секунд
		И я устанавливаю флаг с именем 'ОтгрузкаВДеньги'
